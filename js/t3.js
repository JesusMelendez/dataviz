const URL3 = './js/t3.json';

/****  main functions *****/

//fetch
async function getJson3(liga){
    let response = await fetch(liga);
    let datos = await response.json();
    return datos;
};

function arregloYears3(arregloUnido){
    let x = [];
...
function crearOpciones3(arregloYear,Dropdown){
...
async function surface(){
    let API_PR = await getJson3(URL3);
    ...
    cifra_proporcion.innerHTML = proporcion_base[0].surface.toLocaleString()


    let years = arregloYears3(proporcion);
    let dropdown =  document.getElementById('select_surface');
    crearOpciones3(years,dropdown);
    ...
    dropdown.addEventListener('change', function() {
        ...
        cifra_proporcion.innerHTML = total_tons_ciclo_selected[0].surface.toLocaleString();
...
async function capacitacion3(){

    let API = await getJson3(URL3);
    let kpi_set_gen = API.KPI_SET[1].OBSERVATIONS;
  ...
  let kpi_set_rol = API.KPI_SET[2].OBSERVATIONS;
  ...
  let years = arregloYears3(arregloUnido)
  let dropDownList =  document.getElementById('select_capacitacion');
  crearOpciones3(years,dropDownList);
